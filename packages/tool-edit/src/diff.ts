 ...
 *
 * Provides diff string generation and the replace-mode edit logic used by
 * replace.ts. The original imported `diffLineRuns`/`diffLines` and
 * `structuredPatchHunks` from `@oh-my-pi/pi-natives`; they are reimplemented
 * here as a compact linear-space LCS (Hirschberg-style) line diff plus a
 * unified-hunk emitter, so the package stays dependency-light.
 ...
 */
import { findMatch } from './replace.ts'
import { DEFAULT_FUZZY_THRESHOLD } from './replace.ts'
import type { BlockContextSource } from './block-context.ts'
import { findBlockContextLines } from './block-context.ts'
import { adjustIndentation, normalizeToLF } from './normalize.ts'
...
// ═══════════════════════════════════════════════════════════════════════════
// Line diff machinery (hand-rolled)
// ═══════════════════════════════════════════════════════════════════════════

/** One primitive edit-script step in absolute coordinates. */
type Op =
  | { type: 'keep'; a: number; count: number }
  | { type: 'del'; a: number; count: number }
  | { type: 'ins'; a: number; b: number; count: number }

/** LCS lengths of every prefix of `y` against all of `x`, as one row. */
function lcsLengths(x: string[], y: string[]): Int32Array {
  // row[j] = LCS over consumed x lines and y[0..j) — roll columns only.
  const row = new Int32Array(y.length + 1)
  for (const xLine of x) {
    for (let j = y.length - 1; j >= 0; j--) {
      row[j + 1] = xLine === (y[j] ?? '') ? (row[j] ?? 0) + 1 : Math.max(row[j + 1] ?? 0, row[j] ?? 0)
    }
  }
  return row
}
...
/**
 * Divide-and-conquer longest-common-subsequence diff (Hirschberg's
 * linear-space algorithm). Emits `Op`s directly into `out`. `aLo`/`aHi` are
 * absolute indices into `a`, `bLo`/`bHi` into `b`. Guaranteed O(|a|·|b|) time
 * and O(|a|+|b|) space.
 */
function diffRange(a: string[], aLo: number, aHi: number, b: string[], bLo: number, bHi: number, out: Op[]): void {
  // Trim a common prefix (mutual single-line prefix of the whole file).
  let n = aHi - aLo
  let m = bHi - bLo
  while (n > 0 && m > 0 && a[aLo] === b[bLo]) {
    out.push({ type: 'keep', a: aLo, count: 1 })
    aLo++
    bLo++
    n--
    m--
  }
  // Trim a common suffix.
  while (n > 0 && m > 0 && a[aLo + n - 1] === b[bLo + m - 1]) {
    n--
    m--
  }
  if (n === 0) {
    if (m > 0) out.push({ type: 'ins', a: aLo, b: bLo, count: m })
    return
  }
  if (m === 0) {
  ...
  // Hirschberg split: partition `b` where the first half of `a` and the
  // reversed second half of `a` best align.
  const mid = n >> 1
  const midAbs = aLo + mid
  const forward = lcsLengths(
    a.slice(aLo, midAbs),
    b.slice(bLo, bLo + m),
  )
  const backA = a.slice(midAbs, aHi).slice().reverse()
  const backB = b.slice(bLo, bLo + m).slice().reverse()
  const backward = lcsLengths(backA, backB)
  let bestJ = 0
  let best = -1
  for (let j = 0; j <= m; j++) {
    const score = (forward[j] ?? 0) + (backward[m - j] ?? 0)
    if (score > best) {
      best = score
      bestJ = j
    }
  }
  diffRange(a, aLo, midAbs, b, bLo, bLo + bestJ, out)
  diffRange(a, midAbs, aHi, b, bLo + bestJ, bHi, out)
}

/**
 * Produce a line diff as jsdiff-style parts consumed by
 * {@link generateDiffString}: runs of context lines, then added / removed
 * runs, alternating.
 */
export function diffLines(oldText: string, newText: string): Array<{ value: string; added?: boolean; removed?: boolean }> {
  const a = oldText.split('\n')
  const b = newText.split('\n')
  const ops: Op[] = []
  diffRange(a, 0, a.length, b, 0, b.length, ops)
  const parts: Array<{ value: string; added?: boolean; removed?: boolean }> = []
  let mode: 'none' | 'context' | 'add' | 'remove' = 'none'
  let buffer: string[] = []
  const flush = (): void => {
    if (buffer.length === 0) return
    if (mode === 'context') parts.push({ value: buffer.join('\n') })
    else if (mode === 'add') parts.push({ value: buffer.join('\n'), added: true })
    else if (mode === 'remove') parts.push({ value: buffer.join('\n'), removed: true })
    buffer = []
  }
  for (const op of ops) {
    if (op.type === 'keep') {
      ...
      for (const line of a.slice(op.a, op.a + op.count)) buffer.push(line)
    } else if (op.type === 'del') {
      if (mode !== 'remove') {
        flush()
        mode = 'remove'
      ...
      for (const line of b.slice(op.b, op.b + op.count)) buffer.push(line)
    }
  }
  flush()
  return parts
...
/** A single unified hunk before serialization. */
...
interface ChangeRun {
  oldFrom: number // absolute old index of first changed line
  oldTo: number // absolute old index one past last changed line
  newFrom: number
  newTo: number
}

/**
 * Emit unified diff hunks (GNU-style `@@ -a,b +c,d @@` headers) over a diff
 * of `oldText` and `newText`, with `contextLines` context lines per change
 * run. Adjacent runs merge when their context windows overlap.
 */
export function structuredPatchHunks(oldText: string, newText: string, contextLines: number): PatchHunk[] {
  const a = oldText.split('\n')
  const b = newText.split('\n')
  const ops: Op[] = []
  diffRange(a, 0, a.length, b, 0, b.length, ops)

  // Walk ops into change runs (contiguous del/ins with no keep between).
  ...
  for (const op of ops) {
    ...
    if (op.type === 'del') {
      const run: ChangeRun = { oldFrom: op.a, oldTo: op.a + op.count, newFrom: bIdx, newTo: bIdx }
      const last = runs[runs.length - 1]
      if (last && last.oldTo === op.a && last.newTo === bIdx) {
    ...
    const run: ChangeRun = { oldFrom: aIdx, oldTo: aIdx, newFrom: op.b, newTo: op.b + op.count }
    const last = runs[runs.length - 1]
    if (last && last.oldTo === aIdx && last.newTo === op.b) {
      last.oldTo = run.oldTo
  ...
  // Merge runs whose context windows overlap: the gap of kept lines between
  // run[i]'s end and run[i+1]'s start must be < 2 * contextLines.
  const merged: ChangeRun[] = []
  for (const run of runs) {
    const last = merged[merged.length - 1]
    if (last) {
      const gap = (run.oldFrom - last.oldTo) + (run.newFrom - last.newTo)
      if (gap < Math.max(0, 2 * contextLines)) {
        last.oldTo = run.oldTo
        last.newTo = run.newTo
        continue
      }
  ...
  // Emit. Context of last run must extend as far as newFrom allows.
  const hunks: PatchHunk[] = []
  for (const run of merged) {
    const pre = Math.max(0, run.oldFrom - contextLines)
    const postA = Math.min(a.length, run.oldTo + contextLines)
    const postB = Math.min(b.length, run.newTo + contextLines)
    ...
    let newCount = 0
    // context before
    for (let i = pre; i < run.oldFrom; i++) {
      lines.push(` ${a[i] ?? ''}`)
      oldCount++
      newCount++
    }
    // the change itself: old side then new side
    for (let i = run.oldFrom; i < run.oldTo; i++) {
      lines.push(`-${a[i] ?? ''}`)
      oldCount++
    }
    for (let j = run.newFrom; j < run.newTo; j++) {
      lines.push(`+${b[j] ?? ''}`)
      newCount++
    }
    // Context after — min of the two continuations so both sides stay paired
    // child line numbers stay correct (a line emitted on both sides shifts
    // both counts together).
    const commonTail = Math.min(postA - run.oldTo, postB - run.newTo)
    for (let k = 0; k < commonTail; k++) {
      const aLine = a[run.oldTo + k] ?? ''
      lines.push(` ${aLine}`)
      oldCount++
      newCount++
    }
...
/**
 * Drop gap rows that no longer separate anything (adapted from the original —
 * context rows inserted one at a time can leave stale gaps).
 */
 ...
 * bracket, …) into a numbered diff. Adapted: uses only the lexical bracket
 * scanner — the original preferred tree-sitter when available.
 */
...
/**
 * Generate a numbered unified diff string with line numbers and context.
...
function parseUnifiedHunkHeader(line: string): UnifiedHunkHeader | undefined {
  ...
  return {
    ...
    newStartLine,
    newLineCount,
    ...(changeContext && changeContext.length > 0 ? { changeContext } : {}),
  }
}
...
export function parseDiffHunks(diff: string): DiffHunk[] {
  ...
  while (i < lines.length) {
    const line = lines[i]
    const trimmed = line === undefined ? '' : line.trim()
  ...
  if (options.all) {
    ...
    while (true) {
      ...
      const shouldUseClosest =
        options.fuzzy &&
        (matchOutcome.closest?.confidence ?? 0) >= threshold &&
...
/** Resolve a file path against the tool working directory (local resolver). */
