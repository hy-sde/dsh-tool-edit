/**
 * Fuzzy matching utilities for the edit tool: replace mode algorithms and
 * execution. Provides character-level and line-level fuzzy matching with
 * progressive fallback strategies, occurrence previews, and the harness
 * adapter (`FileReader`/writer) that replaces oh-my-pi's session internals.
...
import { generateDiffString, replaceText } from './diff.ts'
import {
...
export interface ContextLineResult {
  ...
  strategy?: ContextMatchStrategy | undefined
}

/// The "match" attempt plays; the "usage" hashing a filename.
...
function computeRelativeIndentDepths(lines: string[]): number[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    if ((lines[i] ?? '').trim().length > 0) {
      nonEmptyIndents.push(indents[i] ?? 0)
    }
  }
  ...
  return lines.map((line, index) => {
    ...
    const relativeIndent = (indents[index] ?? 0) - minIndent
...
function findBestFuzzyMatchCore(
  ...
  for (let start = 0; start <= contentLines.length - targetLines.length; start++) {
    const startIndex = offsets[start] ?? 0
    const endLine = start + targetLines.length - 1
    const endIndex = Math.max(startIndex + 1, (offsets[endLine] ?? 0) + (contentLines[endLine] ?? '').length)
 ...
 * average early (always ≤ the true score).
 */
...
export function seekSequence(
  ...
  return {
    index: undefined,
    confidence: bestScore,
    ...(fallbackMatchCount === undefined ? {} : { matchCount: fallbackMatchCount }),
  }
}
...
/** Aggregate metadata bag returned alongside the diff (adapted from outputMeta). */
...
/**
 * Execute the replace edit. Adapters vs the original: file reads go through
 * `reader`, writes through `writer` (ctx.fs + fs/edit-intent + fs/observed);
 * plan-mode redirect, ACP bridge and fs-cache invalidation are no-ops here.
 */
export async function executeReplace(options: ExecuteReplaceOptions): Promise<ReplaceOutcome> {
  const { session, path, params, signal, allowFuzzy, fuzzyThreshold, writethrough } = options
  ...
  const absolutePath = await session.reader.resolve(path, signal)
  const rawContent = await session.reader.readText(absolutePath, signal)
  ...
  if (result.count === 0) {
    ...
    throw new EditMatchError(path, normalizedOldText, matchOutcome.closest, {
      allowFuzzy,
      threshold: fuzzyThreshold,
      ...(matchOutcome.fuzzyMatches === undefined ? {} : { fuzzyMatches: matchOutcome.fuzzyMatches }),
    })
  }
...
// widen the interface so diagnostics can ride along
export interface ReplaceOutcomeMaybeDiagnostics extends ReplaceOutcome {
  diagnostics?: import('./lsp/writethrough.ts').EditDiagnosticsResult
}
