 ...
 *
 * The original obtained its file hash from `Bun.hash.xxHash32`. This port
 * uses the standalone replacement from `./hash` (see {@link xxhash32}), so
 * tags stay deterministic on plain Node without a native addon.
 *
...
export function computeFileHash(text: string): string {
  ...
  const low16 = xxhash32(0, encoder.encode(normalized)) & 0xffff
