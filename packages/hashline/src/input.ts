...
import { HL_FILE_HASH_EXAMPLES, HL_FILE_HASH_LENGTH, HL_FILE_HASH_SEP, HL_FILE_PREFIX, HL_FILE_SUFFIX } from './format.ts'
...
function normalizeHashlinePath(rawPath: string, cwd?: string): string {
  ...
  if (!cwd || !pathModule.isAbsolute(unquoted)) return unquoted
  const relative = pathModule.relative(pathModule.resolve(cwd), pathModule.resolve(unquoted))
  const normalizedRelative = relative.split(pathModule.sep).join('/')
  const isWithinCwd = relative === '' || (!relative.startsWith('..') && !pathModule.isAbsolute(relative))
...
export class PatchSection {
  ...
  /**
	 ...
	 * path does not exist onto the file whose snapshot tag actually names.
	 */
