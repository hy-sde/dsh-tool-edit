...
describe('Patcher snapshot tag integrity', () => {
  ...
  it('restores a UTF-8 BOM hidden by Bun text decoding', async () => {
    ...
    try {
      ...
      // The original wrote through Bun.write; node:fs/promises writeFile
      // accepts the same raw UTF-8 bytes.
      await fs.writeFile(filePath, new Uint8Array([0xef, 0xbb, 0xbf, ...new TextEncoder().encode(source)]))
      const snapshots = new InMemorySnapshotStore()
  ...
  it('applies onto live content when the tag matches, even against a retained colliding snapshot', async () => {
    // These two texts both hash to `258E` (spec-compliant xxHash32; the
    // upstream pair was tuned to Bun.hash.xxHash32 and does not collide here).
...
describe('Patcher mandatory snapshot tag policy', () => {
  ...
  it('still hard-rejects an anchored edit that omits the snapshot tag', async () => {
    ...
    await expect(patcher.apply(Patch.parse(`[${PATH}]\nPUT 1-1:\n+X`))).rejects.toThrow(/Missing hashline snapshot tag/)
  })
